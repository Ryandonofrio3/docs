...
description: Use the HTTP API directly when you don't have an SDK available
---

# Direct HTTP API

Use the HTTP API directly when you don't have an SDK available for your language.
...
<AccordionGroup>
  <Accordion title="Batch events">
    Send multiple events in a single request when possible to reduce overhead.
  </Accordion>
  <Accordion title="Fire and forget">
    Don't block on API responses in production. Use async/background processing.
  </Accordion>
  <Accordion title="Generate unique IDs">
    Use UUIDs or similar for `event_id` to ensure uniqueness.
  </Accordion>
  <Accordion title="Include timestamps">
    Always include ISO 8601 timestamps with timezone info.
  </Accordion>
  <Accordion title="Link conversations">
    Use `convo_id` to group related interactions in the same thread.
  </Accordion>
</AccordionGroup>
